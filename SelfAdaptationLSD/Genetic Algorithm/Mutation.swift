...
extension GeneticAlgorithm {
    ...
    func MutatePopulation() {
        for (index, offspring) in offspringPopulation.enumerated() {
            ...
            if randomNumber != 1 {
                let center = max(0.1, 50 * Double(offspring.frontNumber) / Double(paretoFronts.count))
                offspringPopulation[index].UpdateStrictness(globalStrictness : averageStrictness, center: center, upperBound: 50)
            }
            switch randomNumber {
