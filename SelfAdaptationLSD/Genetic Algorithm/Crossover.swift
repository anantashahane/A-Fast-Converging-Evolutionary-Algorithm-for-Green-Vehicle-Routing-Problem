...
extension GeneticAlgorithm {
    ...
    private func Crossover(parent1 : Routine, parent2 : Routine) -> Routine {
        ...
        if remainingCustomers.isEmpty {
            var returnRoutine = Routine(trucks: offspringTrucks, averageStrictness: (parent1.strictness + parent2.strictness) / 2)
            returnRoutine.frontNumber = parent1.frontNumber
