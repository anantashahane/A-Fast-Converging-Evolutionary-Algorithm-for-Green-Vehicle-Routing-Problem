    ...
    func MutatePopulation() {
        for (index, individual) in self.offspringPopulation.enumerated() {
            switch Double.random(in: 0...1) {
                ...
                offspringPopulation[index] = LNS(individual: individual, destructionProbability: destructionProbability, dynamicAnchoring: individual.GetStrictness() < Double(self.benchmark.points.count) / 2)
            }
        }
    }
