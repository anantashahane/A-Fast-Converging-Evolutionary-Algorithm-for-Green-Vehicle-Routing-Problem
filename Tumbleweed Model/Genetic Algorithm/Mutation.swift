...
extension GeneticAlgorithm {
    ...
    func MutatePopulation() {
        ...
        for individual in parentPopulation {
            let randomNumber = SpinRouletteWheel(strictness: 2 * Double(remainingIterations) / Double(iterationCount), onCandidates: Array(1...6))
            switch randomNumber {
            ...
            case 5: newOffspringPopulation.append(CustomerTransferMutation(individual: individual))
            default: newOffspringPopulation.append(FuelOptimiser(individual: individual))
            }
        }
